#!/usr/bin/env python3
"""Run GenAR inference and evaluation from a saved checkpoint."""
...
from safe_io import public_filename, safe_torch_load
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'Evaluate a slide outside the checkpoint test split. Training-count '
            'diagnostics still use the split stored in the checkpoint.'
        ),
    )
...
def _safe_checkpoint_load(
    ...
    """Load a checkpoint without enabling arbitrary Python objects."""
...
def load_model_from_checkpoint(
    ...
    """Restore the model and experiment configuration from a checkpoint."""
    if not os.path.isfile(checkpoint_path):
        raise FileNotFoundError(f"Checkpoint not found: {checkpoint_path}")
    checkpoint = _safe_checkpoint_load(checkpoint_path)
...
def validate_checkpoint_contract(
    ...
    """Check dataset, encoder, and vocabulary metadata."""
    required = [
        'expr_name',
        'encoder_name',
        'max_gene_count',
...
def checkpoint_training_slides(config, dataset: STDataset) -> list[str]:
    """Read the training split stored in the checkpoint."""
...
def save_results(
    ...
    summary = {
        ...
        'slide_id': slide_id,
        'checkpoint_file': public_filename(checkpoint_path),
        'prediction_mode': str(config.MODEL.get('prediction_mode', 'discrete')),
...
def main() -> int:
    ...
    dispersion_source = (
        public_filename(args.dispersion_file)
