#!/usr/bin/env python3
"""Train GenAR and the ablation configurations used in the paper."""
...
def validate_scale_dims(
    ...
    """Validate an ordered coarse-to-fine scale layout."""
...
def get_parse() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Train GenAR with the configuration used in the paper',
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            'Per-process DataLoader batch size. When omitted, the global batch '
            'size is divided evenly across GPU processes.'
        ),
    )
    parser.add_argument(
        '--global-batch-size',
        type=int,
        help=(
            'Batch size across all GPU processes. Defaults to the paper value '
    ...
    parser.add_argument(
        '--scale-learning-rate',
        action='store_true',
        help='Apply linear learning-rate scaling with the global batch size',
    )
    parser.add_argument(
        '--patience',
        type=int,
    ...
    parser.add_argument(
        ...
        action='store_true',
        help='Disable deterministic Trainer mode',
    )
    parser.add_argument(
        '--max-gene-count',
        type=int,
    ...
    parser.add_argument(
        '--val-slides',
        help=(
            'Optional comma-separated validation slides. The paper default '
            'uses no validation slide and selects checkpoints by training loss.'
        ),
    )
    parser.add_argument(
        '--test-slides',
        help='Comma-separated held-out test slides; defaults to the paper split',
    )
    parser.add_argument(
        '--ablation-protocol',
        default='normalized',
        choices=['normalized', 'published'],
        help=(
            'normalized matches per-scale loss reduction and activates the '
            'full continuous head; published reproduces the original Table '
            '4/6 ablation implementation'
        ),
    )
    parser.add_argument(
        '--config',
        help='Deprecated; use explicit command-line options instead',
    )
    return parser
...
def build_config_from_args(args) -> Dict:
    ...
    if model_name == 'GENAR':
        ...
        if (
            ...
            and (
                args.prediction_mode == 'continuous'
                or final_loss_mode == 'cross_entropy'
            )
        ):
            run_parts.append('published-protocol')
    ...
    config.slide_val = (
        ...
        if args.val_slides is None
        else args.val_slides
    )
    config.slide_test = (
        dataset_info['test_slides']
        if args.test_slides is None
        else args.test_slides
    )
    val_slides = {
        slide.strip()
        for slide in config.slide_val.split(',')
        if slide.strip()
    }
    test_slides = {
        slide.strip()
        for slide in config.slide_test.split(',')
        if slide.strip()
    }
    overlap = val_slides.intersection(test_slides)
    if overlap:
        raise ValueError(
            "Validation and test slides must be disjoint: "
            + ', '.join(sorted(overlap))
        )
    if not test_slides:
        raise ValueError("At least one test slide is required")
...
def create_dataloaders(config: Dict):
    """Build loaders without exposing the test split during training."""
    base_params = {
    ...
    if config.mode == 'train':
        reference_dataset = STDataset(mode='train', **base_params)
        train_loader = DataLoader(
            reference_dataset,
            **_loader_kwargs(config, 'train'),
        )
        if str(config.slide_val).strip():
            val_dataset = STDataset(mode='val', **base_params)
            if val_dataset.genes != reference_dataset.genes:
                raise ValueError(
                    "Training and validation splits have different gene orders"
                )
            val_loader = DataLoader(
                val_dataset,
                **_loader_kwargs(config, 'val'),
            )
    else:
        reference_dataset = STDataset(mode='test', **base_params)
        test_loader = DataLoader(
            reference_dataset,
            **_loader_kwargs(config, 'test'),
        )

    ordered_genes = '\n'.join(reference_dataset.genes) + '\n'
    config.DATA.data_contract_version = 1
    config.DATA.selected_gene_order_sha256 = hashlib.sha256(
        ordered_genes.encode('utf-8')
    ).hexdigest()
    config.DATA.train_slides = list(reference_dataset.slide_splits['train'])
    config.DATA.val_slides = list(reference_dataset.slide_splits['val'])
    config.DATA.test_slides = list(reference_dataset.slide_splits['test'])
...
def main(config: Dict):
    ...
    if config.mode == 'train':
        ...
        if test_loader is None:
            raise RuntimeError("Test loader was not created")
