...
class ModelInterface(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # Normalize checkpoint/plain-dict configurations to recursive
        # attribute access while storing only built-in containers in hparams.
        self.config = AddictDict(config)
        serializable_config = self.config.to_dict()
        self._assert_config_serializable(serializable_config)
        checkpoint_config = deepcopy(serializable_config)
        checkpoint_config.pop('data_path', None)
        checkpoint_config.pop('ckpt_path', None)

        # Persist only serializable hyperparameters
        hyperparams = {
            'config': checkpoint_config,
            'checkpoint_schema_version': 1,
            'model_name': getattr(self.config.MODEL, 'model_name', 'GENAR'),
            'num_genes': getattr(self.config.MODEL, 'num_genes', 200),
            'learning_rate': getattr(self.config.TRAINING, 'learning_rate', 1e-4),
            'batch_size': getattr(self.config.DATA.train_dataloader, 'batch_size', 64),
            'max_epochs': getattr(self.config.TRAINING, 'num_epochs', 200),
            'dataset': getattr(self.config, 'expr_name', 'unknown')
        }
        self.save_hyperparameters(hyperparams)

        # Helper utilities
        self.model_utils = ModelUtils(self.config, self)
        self.model_metrics = ModelMetrics(self.config, self)
        ...
        self.inference_top_k = self.config.INFERENCE.top_k
    ...
    def _common_step(self, batch, batch_idx, phase: str) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if phase in ['val', 'test']:
            ...
            with torch.no_grad():
                ...
                self.model.eval()
                # Paper evaluation uses top-k=1, which is deterministic argmax.
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_size = int(batch['target_genes'].shape[0])
        ...
        # Keep one unambiguous epoch-level key for the scheduler/checkpoint.
        self.log(
            'train_loss_final',
            loss_final,
            on_step=False,
    ...
    def _compute_loss(self, outputs: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        try:
            ...
            self._logger.error(f"Failed to compute loss: {e!s}")
    ...
    def configure_optimizers(self):
        ...
        if bool(self.model_utils.get_config('TRAINING.scale_learning_rate', False)):
    ...
    def _compute_and_log_pcc_metrics(self, phase: str):
        ...
        pcc_metrics = self.model_metrics.calculate_comprehensive_pcc_metrics(
            predictions,
            targets,
            apply_log2=self.model_metrics.should_apply_log2(),
        )
    ...
    def on_save_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        """Store the checkpoint schema version."""
        checkpoint['genar_checkpoint_schema_version'] = 1
