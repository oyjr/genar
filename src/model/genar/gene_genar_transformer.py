"""Transformer blocks used by the GenAR decoder."""
...
class GeneAdaLNBeforeHead(nn.Module):
    """Adaptive layer normalization before the output head."""
    ...
    def forward(self, x: torch.Tensor, condition_embed: torch.Tensor) -> torch.Tensor:
        """Apply adaptive layer normalization to ``x``."""
...
class ConditionProcessor(nn.Module):
    """Fuse histology features with encoded spatial coordinates."""
    def __init__(
        self,
