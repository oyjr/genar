...
class MultiScaleGenAR(nn.Module):
    ...
    def __init__(
        ...
        # Keep the same parameterized head and token embedding in the
        # architecture-matched regression control. Continuous mode uses one
        # channel as its scalar regression head and interpolates the shared
        # token embedding for previous values, so parameter counts are exact.
    ...
    def _continuous_head(self, hidden: torch.Tensor) -> torch.Tensor:
        """Use one channel of the parameter-matched output matrix as a regressor."""
        return self.output_head(hidden)[..., 0]
