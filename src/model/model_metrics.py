    ...
    def calculate_gene_correlations(self, y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
        ...
        for i in range(num_genes):
            ...
            if np.std(true_gene) == 0 or np.std(pred_gene) == 0:
                raise ValueError(f"Zero-variance gene encountered at index {i}")

            corr = np.corrcoef(true_gene, pred_gene)[0, 1]
            if np.isnan(corr):
                raise ValueError(f"NaN correlation for gene index {i}")
            correlations[i] = corr
    ...
    def calculate_comprehensive_pcc_metrics(self, predictions: torch.Tensor, targets: torch.Tensor, 
        ...
        if apply_log2:
            ...
            if np.isnan(y_true_log2).any() or np.isnan(y_pred_log2).any():
                raise ValueError("NaN detected after log2 transform")
        ...
        if np.any(true_var <= 0):
            raise ValueError("Zero or negative variance detected in target genes for RVD computation")

        rvd = np.mean(((pred_var - true_var) ** 2) / (true_var ** 2))
