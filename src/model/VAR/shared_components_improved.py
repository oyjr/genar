...
class SharedVectorQuantizer(nn.Module):
    ...
    def __init__(
        ...
        print(f"🔧 SharedVectorQuantizer改进版初始化:")
        print(f"   词汇表大小: {vocab_size}")
        print(f"   嵌入维度: {embed_dim}")
        print(f"   β参数: {beta} (原来0.25)")
        print(f"   初始化std: {std} (原来{1/vocab_size:.6f})")
        print(f"   EMA更新: {use_ema}")
    ...
    def _ema_update(self, flat_x: torch.Tensor, tokens_flat: torch.Tensor):
        ...
        embed_sum.index_add_(0, tokens_flat, flat_x)
