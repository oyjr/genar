"""Baseline predictor using fixed histology embeddings."""
...
class FoundationOnlyRegressor(nn.Module):
    """Predict gene tokens with a small MLP.

    The class follows the GenAR model interface so it can use the same
    Lightning training and evaluation code.
    """
    ...
    def forward(
        ...
        **unused_kwargs,
        ...
        # Flatten optional spot dimensions before the MLP.
        ...
        for layer in self.hidden_layers:
            ...
            x = self.activation(x)
            x = self.dropout(x)
            x = x + residual  # residual connection to ease optimisation
