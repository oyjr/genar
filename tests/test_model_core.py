...
class MultiScaleCoreTests(unittest.TestCase):
    ...
    def test_continuous_regression_training_and_inference(self):
        ...
        self.assertTrue(torch.all(inference['generated_sequence'] >= 0))

    def test_normalized_ablation_uses_per_sample_position_sum(self):
        predictions = torch.tensor([[1.0, 2.0], [3.0, 4.0]])
        targets = torch.zeros_like(predictions)
        normalized = tiny_model(
            prediction_mode='continuous',
            ablation_protocol='normalized',
        )
        published = tiny_model(
            prediction_mode='continuous',
            ablation_protocol='published',
        )
        self.assertTrue(
            torch.allclose(
                normalized._compute_continuous_loss(predictions, targets),
                predictions.square().sum(dim=1).mean(),
            )
        )
        self.assertTrue(
            torch.allclose(
                published._compute_continuous_loss(predictions, targets),
                predictions.square().mean(),
            )
        )

    def test_normalized_cross_entropy_matches_kl_scale_reduction(self):
        logits = torch.randn(2, 4, 9)
        targets = torch.randint(0, 9, (2, 4))
        normalized = tiny_model(
            final_loss_mode='cross_entropy',
            ablation_protocol='normalized',
        )
        published = tiny_model(
            final_loss_mode='cross_entropy',
        ...
        self.assertTrue(
            torch.allclose(
                normalized._compute_final_scale_loss(logits, targets),
                tokenwise.sum(dim=1).mean(),
            )
        )
        self.assertTrue(
            torch.allclose(
                published._compute_final_scale_loss(logits, targets),
                tokenwise.mean(),
            )
        )

    def test_normalized_continuous_head_activates_every_output_channel(self):
        model = tiny_model(
            prediction_mode='continuous',
            ablation_protocol='normalized',
        )
        model.zero_grad(set_to_none=True)
        model._continuous_head(torch.randn(2, 3, 4)).sum().backward()
        self.assertIsNotNone(model.output_head.bias.grad)
        self.assertTrue(torch.all(model.output_head.bias.grad != 0))
        ...
        self.assertTrue(
            torch.equal(
                published._continuous_head(hidden),
                published.output_head(hidden)[..., 0],
            )
        )
    ...
    def test_no_film_checkpoint_preserves_nondefault_architecture(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(payload.pop('ablation_protocol'), 'normalized')
        ...
        self.assertEqual(restored.ablation_protocol, 'published')
