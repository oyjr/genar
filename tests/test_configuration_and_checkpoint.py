...
class ConfigurationAndCheckpointTests(unittest.TestCase):
    def test_final_paper_defaults(self):
        ...
        self.assertEqual(config.MODEL.ablation_protocol, 'normalized')
        self.assertEqual(config.slide_val, '')
        self.assertEqual(config.slide_test, 'MEND145')
    ...
    def test_scale_validation_rejects_invalid_coarse_to_fine_layouts(self):
        self.assertEqual(
    ...
    def test_validation_and_test_overrides_must_be_disjoint(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            (root / 'PRAD').mkdir()
            args = get_parse().parse_args(
    ...
    def test_training_dataloader_does_not_construct_test_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            config = build_config_from_args(args)
            with mock.patch('main.STDataset', DatasetStub):
                train, validation, test = create_dataloaders(config)
        self.assertIsNotNone(train)
        self.assertIsNone(validation)
        self.assertIsNone(test)
        self.assertEqual(DatasetStub.modes, ['train'])
    ...
    def test_checkpoint_config_does_not_store_local_paths(self):
        config = tiny_interface_config()
        config.data_path = '/example/dataset'
        config.ckpt_path = '/example/model.ckpt'
        module = ModelInterface(config)
        persisted = module.hparams['config']
        self.assertNotIn('data_path', persisted)
        self.assertNotIn('ckpt_path', persisted)

    def test_legacy_checkpoint_config_uses_published_ablation_protocol(self):
        config = tiny_interface_config()
        del config.MODEL['ablation_protocol']
        module = ModelInterface(config)
        self.assertEqual(module.model.ablation_protocol, 'published')
